...
| `VerifyAcceptanceCriteriaTool.ts`      | `src/core/tools/VerifyAcceptanceCriteriaTool.ts` | —     | Marks intent COMPLETE when acceptance criteria verified; IN_PROGRESS→COMPLETE |
...
**Implemented (previously planned):** HITL Authorization Gate (§3.1), `.intentignore` (§5.3), CLAUDE.md Auto-Append via LessonRecorder (§4.4), **Acceptance Criteria Verification** (verify_acceptance_criteria tool, IN_PROGRESS→COMPLETE transition)
